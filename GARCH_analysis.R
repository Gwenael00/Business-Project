...
var_mat_sGARCH =matrix(,2768-S,m*n)
colnames(var_mat_sGARCH) <- c("1,1","1,2","1,3","2,1","2,2","2,3","3,1","3,2","3,3")
rownames(var_mat_sGARCH) <- NULL

realized_ret <- sp500ret[(S+1):2768]
indic_var_sGARCH = matrix(,2768-S,m*n)
...
for (i in 1:m) {
  for (j in 1:n) {
    ...
    garchfit <- ugarchfit(spec = garchspec,data = sp500ret, out.sample = 1008, fit.control = list(stationarity = 1))
    # show(garchfit)
