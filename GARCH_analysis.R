...
input_data = read.csv("/Users/benita/Documents/Benita/Uni/CBS/Semester 2/Business Project/R/qsidefvvgiaieddt.csv")
...
var_mat_sGARCH =matrix(,5284-S,m*n)
colnames(var_mat_sGARCH) <- c("1,1","1,2","1,3","2,1","2,2","2,3","3,1","3,2","3,3")
rownames(var_mat_sGARCH) <- NULL

realized_ret <- sp500ret[(S+1):5284]
indic_var_sGARCH = matrix(,5284-S,m*n)
...
for (i in 1:m) {
  for (j in 1:n) {
    ...
    garchfit <- ugarchfit(spec = garchspec,data = sp500ret, out.sample = 1000, fit.control = list(stationarity = 1))
    # show(garchfit)
   ...
   # rolling OOS prediction
    garch_pred <- ugarchroll(spec=garchspec, data=sp500ret, n.ahead = 1, n.start = S, 
                   ...
                   window.size = 1500, calculate.VaR = TRUE, VaR.alpha = c(0.01, 
                                                                                0.05), keep.coef = TRUE, ) 
...
# find uncond coverage through frequency of exceedence
for (i in 1:m) {
  for (j in 1:n) {
   # uncond_f_sGARCH[1,(i-1)*3+j] <- mean(indic_var_sGARCH[,(i-1)*3+j]) 
...
var_mat_sGARCH_dist =matrix(,5284-S,3)
colnames(var_mat_sGARCH_dist) <- c("normal","t-dist","GED")
rownames(var_mat_sGARCH_dist) <- NULL

realized_ret <- sp500ret[(S+1):5284]
indic_var_sGARCH_dist = matrix(,5284-S,3)
uncond_f_sGARCH_dist = matrix(,1,3)

####Loop through models:

    
    # model specification - normal
    garchspec <- ugarchspec(mean.model = list(armaOrder = c(0,0)),
                            variance.model = list(model = "sGARCH", garchOrder = c(p,q)), 
                            distribution.model = "norm")
    garchfit <- ugarchfit(spec = garchspec,data = sp500ret, out.sample = 1000, fit.control = list(stationarity = 1))
    info_mat_sGARCH_dist[,1] = infocriteria(garchfit)
    garch_pred <- ugarchroll(spec=garchspec, data=sp500ret, n.ahead = 1, n.start = S, 
                             refit.every = 22, refit.window = c( "moving"), 
                             window.size = 1500, calculate.VaR = TRUE, VaR.alpha = c(0.01, 
                                                                                     0.05), keep.coef = TRUE, ) 
    var_mat_sGARCH_dist[,1] <-garch_pred@forecast[["VaR"]][["alpha(1%)"]]
    report(garch_pred, type = "VaR", VaR.alpha = 0.01, conf.level = 0.99)


    # model specification - t-dist
    garchspec <- ugarchspec(mean.model = list(armaOrder = c(0,0)),
                            variance.model = list(model = "sGARCH", garchOrder = c(p,q)), 
                            distribution.model = "std")
    garchfit <- ugarchfit(spec = garchspec,data = sp500ret, out.sample = 1000, fit.control = list(stationarity = 1))
    info_mat_sGARCH_dist[,2] = infocriteria(garchfit)
    garch_pred <- ugarchroll(spec=garchspec, data=sp500ret, n.ahead = 1, n.start = S, 
                             refit.every = 22, refit.window = c( "moving"), 
                             window.size = 1500, calculate.VaR = TRUE, VaR.alpha = c(0.01, 
                                                                                     0.05), keep.coef = TRUE, ) 
    var_mat_sGARCH_dist[,2] <-garch_pred@forecast[["VaR"]][["alpha(1%)"]]
    report(garch_pred, type = "VaR", VaR.alpha = 0.01, conf.level = 0.99)
    
    # model specification - GED
    garchspec <- ugarchspec(mean.model = list(armaOrder = c(0,0)),
                            variance.model = list(model = "sGARCH", garchOrder = c(p,q)), 
                            distribution.model = "ged")
    garchfit <- ugarchfit(spec = garchspec,data = sp500ret, out.sample = 1000, fit.control = list(stationarity = 1))
    info_mat_sGARCH_dist[,3] = infocriteria(garchfit)
    garch_pred <- ugarchroll(spec=garchspec, data=sp500ret, n.ahead = 1, n.start = S, 
                             refit.every = 22, refit.window = c( "moving"), 
                             window.size = 1500, calculate.VaR = TRUE, VaR.alpha = c(0.01, 
                                                                                     0.05), keep.coef = TRUE, ) 
    var_mat_sGARCH_dist[,3] <-garch_pred@forecast[["VaR"]][["alpha(1%)"]]
